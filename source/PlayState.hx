...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			case 'room': //hey guys welcome back to a brand new video and in todays video I am going to be explaining the bluestopsign mod, so basically uhh the among us rips poster kill gf the end :)
				var bg:BGSprite;
				bg = new BGSprite('newbg', -1300, -700, 0.95, 0.95);
				bg.setGraphicSize(Std.int(bg.width * 0.5));
				add(bg);
			
			case 'roomAmogus': //oh shit imposter ripped da poster :(
				var bg:BGSprite;
				bg = new BGSprite('newbgRipped', -1300, -700, 0.95, 0.95);
				bg.setGraphicSize(Std.int(bg.width * 0.5));
				add(bg);
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'back-to-basics':
					startVideo('Cutscene_1_BSS');
				case 'explained':
					startVideo('Cutscene_2_BSS');
				case 'stop':
					startVideo('Cutscene_3_BSS');
